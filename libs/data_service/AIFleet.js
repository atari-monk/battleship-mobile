export class AIFleet {
  constructor() {
    this._matrix = Array(10)
      .fill()
      .map(() => Array(10).fill(0))
    this._ships = [] // Track all ships and their positions
  }
  ...
  hit(x, y) {
    for (const ship of this._ships) {
      for (const [sx, sy] of ship.positions) {
        if (sx === x && sy === y) {
          ship.hits += 1
          this._matrix[x][y] = -1 // Mark hit
          return true
        }
      }
    }
    this._matrix[x][y] = -1 // Mark miss
    return false
  }
  ...
  isPartOfShip(x, y, ship) {
    return ship.positions.some(([sx, sy]) => sx === x && sy === y)
  }
}
